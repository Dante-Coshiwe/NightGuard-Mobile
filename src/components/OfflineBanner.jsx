      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        {isOnline
          ? syncing
            ...
            : `Back online — ${queueCount} item${queueCount !== 1 ? 's' : ''} to sync`
          : `Offline — ${queueCount} item${queueCount !== 1 ? 's' : ''} queued`
        }
      </div>
      {isOnline && queueCount > 0 && !syncing && (
        <button onClick={syncQueue}
          style={{ padding: '4px 12px', background: '#fff', color: '#166534', border: 'none', borderRadius: 6, fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
          Sync Now
        </button>
      )}
    </div>
